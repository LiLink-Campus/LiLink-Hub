...
async function exportJob(contentId: string, outFile: string): Promise<void> {
  // 动态 import，避免非 export 命令也加载 Payload/DB。
  ...
  if (persisted && typeof persisted === 'object') {
    ...
    if (cc.status !== 'ready_to_publish') {
      log(`⚠ 稿件状态为 ${String(cc.status)}（非 ready_to_publish）；仍用已持久化发布包导出`)
    }
  } else {
    ...
    log('⚠ 未找到持久化发布包(manualPackage)，已从当前字段重建（建议先在中台「提交审核→通过」生成发布包）')
  }

  // error 级告警（缺图/缺视频等）阻断导出，避免发出不完整内容。
  ...
  const socialDescription =
    typeof cc.socialDescription === 'string' && cc.socialDescription.trim()
      ...
      : undefined
  const job = buildBrowserJob(pkg, { contentId: String(contentId), body: socialDescription })
...
async function run(args: {
  ...
  const result = await runBrowserPublish(job, {
    mode,
    headed: args.headed,
